...
if [ -f "$WORK_DIR/kernel/dtbo_lte.img" ]; then
    EVAL "rm -f \"$WORK_DIR/kernel/dtbo_lte.img\""
fi
...
EVAL "find \"$KERNEL_TMP_DIR\" -iname 'dtbo_lte.img' -exec cp {} \"$WORK_DIR/kernel/dtbo_lte.img\" \;"
